...
var select = (callback, fields, conditions = "1 = 1") => {
    ...
    conn.connect().then(() => {
        ...
        var query = `SELECT ${fields}, users.first_name, users.last_name, users.faculty_id, topics.name AS topic, topics.end_date, faculties.name AS faculty, contributions.id FROM contributions ` +
